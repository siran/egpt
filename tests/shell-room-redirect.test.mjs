...
//
// "lobby" is the console's OWN home room. Since 2026-08-28 it has a folder like any other
// room (rooms/lobby/) and the shell's surface IS `room` — so joining it means "go home", and
// must still NOT redirect anywhere (spec, verbatim from the brief).
// The NETWORK stays 'shell' throughout: that is the console's authority, and this file is one
// of the places that would notice if it silently changed.
...
import { LOBBY_SLUG as LOBBY } from '../src/conversations-state.mjs';

describe('redirectShellToRoom — the pure decision boot.mjs\'s shell-limb wiring makes', () => {
  const shellMsg = (body = 'hi') => ({ body, from: { chatId: LOBBY, chatName: 'shell', network: 'shell', userId: 'operator' } });

  it('REPRODUCE-FIRST: no room joined (currentRoomOf → null) — the message is returned UNCHANGED, still the console lobby seat', () => {
    const claims = [];
    const msg = shellMsg();
  ...
  it("'lobby' IS the console's own home room (rooms/lobby/) — joining it means 'go home', so it does NOT redirect", () => {
    const claims = [];
  ...
  // the entry that gates the redirect and the shell got permanently wedged inside the room.
  // Commands (anything starting with '/') must NEVER be part of the "prose fan-out to the
  // current room" feature — they always keep operating on the console's own lobby seat,
...
describe('the live gap, reproduced end to end: /room join acim → currentRoomOf → redirect → identity.build', () => {
  ...
  it('after /room join acim, the SAME message redirected + identity.build lands on (room, acim) — E now runs confined to that room', async () => {
    ...
    const identity = createIdentity();
    const inbound = { body: '@e hi E. please execute translation script', from: { chatId: LOBBY, chatName: 'shell', network: 'shell', userId: 'operator', authorized: true, atEStart: true, atEAnywhere: true } };
  ...
  it('/room join lobby restores native lobby routing for subsequent messages', async () => {
    ...
    const identity = createIdentity();
    const inbound = { body: 'hola', from: { chatId: LOBBY, chatName: 'shell', network: 'shell', userId: 'operator', authorized: true } };
    const redirected = redirectShellToRoom(inbound, { currentRoomOf: commands.currentRoomOf, claim: () => {} });
    const ev = identity.build(redirected);
    expect(ev.surface).toBe('room');
    expect(ev.chatId).toBe(LOBBY);
...
describe('outbound routing: the redirect must claim() the room chatId, or a reply never finds its way back to the shell socket', () => {
  ...
  it('end to end: makeShellAwareBridge routes a send for the claimed room chatId to the shell socket, not the raw bridge', async () => {
    ...
    redirectShellToRoom({ body: 'x', from: { chatId: LOBBY, network: 'shell' } }, { currentRoomOf: commands.currentRoomOf, claim: port.claim });
