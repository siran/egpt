...
// ── do: post-0018 and post-0019. Nothing answers to `rodz`; the worker answers to `d`/`don` and,
// since 0018, is keyed by that handle. ───────────────────────────────────────────────────────────
const DO_LINES = [
...
describe('0020 on do - the worker answers to `@rodz` too', () => {
  it('plans exactly one changed line, and says why', async () => {
    ...
    expect(p.changes).toEqual([
      ...
      'agents.don answers to `@rodz` from here on - `@rodz` is the account both nodes speak through, and a mention of it now reaches this node\'s worker',
  ...
  it('apply: `rodz` is added and `d`/`don` are kept, the trailing comment and every other byte untouched', async () => {
    ...
    expect(YAML.parse(readFileSync(cfgPath(h), 'utf8')).agents.don.handles).toEqual(['d', 'don', 'rodz']);
  ...
  it('only `rodz`, never `r` - a one-letter handle is too easy to trigger by accident', async () => {
    ...
    expect(YAML.parse(readFileSync(cfgPath(h), 'utf8')).agents.don.handles).not.toContain('r');
  });

  it('the worker is found by HANDLE: a being KEYED `don` whose handles say otherwise is not it', async () => {
    // The persona is keyed `worker` here purely so the DECOY can take the key `don`: one being
    // KEYED `don` that does not answer to it, one keyed otherwise that does.
    const keyed = DO.replace('  don:', '  worker:').replace('  codex:', '  don:');
    const h = home({ config: keyed, conversations: null, identity: null });
    const p = await plan(ctxFor(h));
    // `worker` still answers to `don` through its own list, and THAT is the being extended.
...
describe('0020: each half is independently satisfiable, and the node that has neither is satisfied', () => {
  ...
  it('already applied on do: satisfied with both notes', async () => {
    ...
    expect(p.notes).toEqual([
      `nothing in ${cfgPath(h)} answers to \`rodz\` but agents.don, which is the being that answers to \`don\` and keeps it, and there is no ${idPath(h)} - nothing to evict here`,
      'agents.don already answers to `rodz`',
    ]);
    expect(readFileSync(cfgPath(h), 'utf8')).toBe(DO_AFTER);
  ...
  it('the being that answers to `don` is NEVER the being removed, even once it answers to `rodz`', async () => {
    ...
    const p = await plan(ctxFor(h));
    expect(p.satisfied).toBe(true);
    expect(p.notes[0]).toBe(`nothing in ${cfgPath(h)} answers to \`rodz\` but agents.don, which is the being that answers to \`don\` and keeps it, and there is no ${idPath(h)} - nothing to evict here`);
    expect(readFileSync(cfgPath(h), 'utf8')).toBe(DO_AFTER);
  });
});
...
describe('0020 refuses, naming the place, only on what it cannot honestly edit', () => {
  ...
  it('two beings answer to `don`', async () => {
    ...
    await expect(plan(ctxFor(h))).rejects
      .toThrow(/0020 refuses: 2 beings in .* answer to `don` \(don, codex\) - which one takes `@rodz` is a human decision, not a guess/);
    expect(readFileSync(cfgPath(h), 'utf8')).toBe(two);
  });
  ...
  it('a `handles:` that is not a list at all', async () => {
    const scalar = DO.replace('    handles: [ d, don ] # the worker, addressed either way', '    handles: don');
  ...
  it('a `handles:` written as a BLOCK list - a shape this splice does not write', async () => {
    ...
    await expect(plan(ctxFor(h))).rejects
      .toThrow(/0020 refuses: `handles:` in agents\.don cannot take `rodz` \(refusing to edit agents\.don\.handles: it is a block list; only an inline flow list is appended to\)/);
...
describe('0020 through the runner', () => {
  ...
  it('kg: applied and recorded; the being, its four records and its identity are gone', async () => {
    const h = home();
    const { exitCode } = await runMigrations({ through: '0020', dir, egptHome: h, elevated: false, platform: 'win32', ctx, log: () => {} });
  ...
  it('EVERY earlier migration reads satisfied on both fixtures - one that acted would invalidate these', async () => {
    for (const h of [home(), doHome()]) {
      await runMigrations({ through: '0020', dir, egptHome: h, elevated: false, platform: 'win32', ctx, log: () => {} });
