...
export const COMMANDS = [
  ...
  { cmd: '/use',            surface: 'both',      usage: '/use [<name>|clear]',                            desc: 'set the session that plain-text routes to (no @-mention needed)' },
  ...
  { cmd: '/mirror',         surface: 'both',      usage: '/mirror @<target> [mN] [--tagged]',              desc: 'forward a message body to a destination without re-running the brain. /mirror @waN sends to a WA chat (from /channels); add mN to pick that specific item by its short id (default = the last visible non-system message). --tagged prefixes the body with [<author> <timestamp>]: so the destination knows it\'s a forward and who originally said it. Shell additionally tracks the forwarded send on the original item, so a subsequent @m<N> reply reaches the new chat too. Extension form (no mN) forwards the last observed inbound to @e / @waN / @session.' },
