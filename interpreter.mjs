...
export const COMMANDS = [
  ...
  { cmd: '/oracle',         surface: 'shell',     usage: '/oracle @waN [--keep] | /oracle stop [@waN] | /oracle list',  desc: 'summon a long-running mystical Oracle in a WA chat — a self-editing spinner message that anyone in the chat can reply to with a question. The bridge routes the question through a brain (default @e, configurable via oracle.brain), edits a 🔮 thinking placeholder into the answer as a reply. Default single-shot: retires after first answer. --keep keeps the oracle alive for multiple Q+A rounds until /oracle stop. Multiple oracles run in parallel across chats; busy_behavior config controls what happens to replies that arrive mid-think. Q+A flows into the shell for operator visibility.' },
