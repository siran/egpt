...
# `/e access regular` points a conversation back at. Parsed fresh on every turn
# by src/spine/permission-levels.mjs — no caching, no freeze; editing this file
# changes behavior immediately for every conversation currently pointing at
# 'regular', with no `/e access` re-run needed.
#
