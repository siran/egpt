...
// /bus-key                — print the current bus signing key (or "none")
// /bus-key gen            — generate a new key + store + print it
// /bus-key set <base64>   — set the key to the supplied value
// /bus-key clear          — remove the key (signing off; permissive verify)
//
// The key is stored in chrome.storage.local under 'bus_key'. The
// bus-ext module auto-loads it on init and re-loads via
// chrome.storage.onChanged — no reload needed for changes to take
// effect. Phase 2 (encryption) will reuse the same key.
//
// ctx:
//   log(text), error(text)
//   storageLocal              ({ get, set, remove })
//   generateKey()             → base64 key string (from tools/bus-sign.mjs)
