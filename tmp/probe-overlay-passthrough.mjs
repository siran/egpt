// Verify: an overlay covering the WA tab with pointer-events:none
// does NOT swallow our Input.dispatchMouseEvent on the chat-list row.
// (Conversely, with pointer-events:auto it does — same as before.)
...
async function open() {
  ...
  return { cmd, evalJs, sleep, close: () => ws.close() };
}

const showOverlay = (pe) => `(() => {
  let el = document.getElementById('__egpt_typing_overlay');
  if (!el) {
    el = document.createElement('div');
    el.id = '__egpt_typing_overlay';
    el.style.cssText = 'position:fixed;inset:0;background:rgba(15,20,25,0.55);color:#fff;font:600 16px system-ui,sans-serif;display:flex;align-items:center;justify-content:center;z-index:2147483647;';
    el.textContent = 'egpt is typing for you…';
    document.body.appendChild(el);
  }
  el.style.pointerEvents = ${JSON.stringify(pe)};
})()`;
const HIDE = `document.getElementById('__egpt_typing_overlay')?.remove()`;
const HEADER = `(() => {
...
async function trySwitch(label, pe) {
  ...
  try {
    ...
    console.log(`${label} (pointer-events:${pe}):  "${before}" → "${after}"  switched=${switched}`);
    return switched;
  } finally {
    await evalJs(HIDE).catch(() => {});
    close();
  }
}

const a = await trySwitch('A', 'auto');
await new Promise(r => setTimeout(r, 500));
const b = await trySwitch('B', 'none');
console.log();
console.log(`pointer-events:auto switched: ${a}  (expect false — overlay swallows click)`);
console.log(`pointer-events:none switched: ${b}  (expect true  — click passes through)`);
