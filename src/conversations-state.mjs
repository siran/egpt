...
export async function seedIdentityLayers(room, name, { io = {}, overwrite = false } = {}) {
  ...
  try {
    ...
    // of the `./transcripts/` dead-end (2026-07-25): a card naming a directory nothing ever
    // created sent E to a path that did not exist. Empty by design: the operator (or E)
    // puts things in; an empty folder tells E honestly that there are none yet.
    await room.ensureTree({ io: { mkdir: mkdirFn } });
...
// LOAD-BEARING: readdirSync is used by listIdentityLayers. It sat under the
// old-spine layout-migration section, which was removed with the old spine — the
// import stays (ESM hoists it, so the mid-file position is harmless).
