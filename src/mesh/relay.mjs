...
export function createMeshRelay({
  ...
  dedupMs = 8_000,             // suppress an identical relay (target+body+return) within this window; 0 disables
  log = () => {},
  now = Date.now,
  random = Math.random,
} = {}) {
  const pending = new Map();   // request_id -> { returnTo, target, noticeTimer, reapTimer }
  const recent = new Map();    // dedup key -> ts: guards a double-dispatched message from double-relaying
  ...
  async function relayOut({ name, toNode, body = '', returnTo = null, target = null } = {}) {
    ...
    // De-dupe: an auto-chat can pass one human message through dispatch twice,
    // which would relay it — and ping the being — twice. Same target+body+return
    // within the window → relay once. (One @don? @wren? produced two identical
    // "thinking…" pings; this kills the second.)
    if (dedupMs > 0) {
      const _dk = `${label}\u001f${body}\u001f${JSON.stringify(returnTo ?? null)}`;
      const _t = now();
      for (const [k, ts] of recent) if (_t - ts > dedupMs) recent.delete(k);
      if (recent.has(_dk)) { log(`mesh: duplicate relay suppressed for ${label}`); return null; }
      recent.set(_dk, _t);
    }
