...
//
// Steps: name → brain → model → effort → identity. `name` is free text (blank = the
// chat's default E); the rest are numbered picks from option lists the spine supplies
// (brains from the siblings/brain registry, models, efforts, identities from identities/).

const STEPS = [
  { key: 'name',        label: 'name / @handle?  (free text · blank = configure E)', free: true },
