...
// Add (or update the kind of) a member. New members default to `muted`.
// `id` is the WA jid / TG chat id / brain name; identity within a room.
export function addMember(state, name, { kind, id, state: memberState = DEFAULT_MEMBER_STATE } = {}) {
  const key = sanitizeName(name);
  ...
  if (existing) { existing.kind = kind; }   // keep its current state on re-add
  else members.push({ kind, id, state: memberState });
  return next;
}
