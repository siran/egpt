...
export const CONFIG_SCHEMA = {
  ...
  whatsapp_cdp: 'extension-only WhatsApp via a content script in web.whatsapp.com (no CDP, no Chrome launch flags). v1: { enabled (set to false to opt out), chat_id (auto-captured), chat_name (REQUIRED for sends — the visible chat-list label of the conversation egpt should send to, e.g. "An" for self-DM or any contact name; if absent, sends go to whatever chat is currently open), client_name (default "wa-cdp") }. Tab presence IS the on/off switch — close the tab to stop. Background republishes incoming WA messages as room-utterance bus events. See BRIDGES_CDP_SPEC.md.',
