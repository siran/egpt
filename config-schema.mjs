...
export const CONFIG_SCHEMA = {
  ...
  whatsapp:     'whatsapp bridge config: { enabled, allowed_users, chat_id (auto-captured), mirror_chat_id (default: self-DM), awareness {...}, client_name (default "wa", appears in handle@client tags), max_backlog_seconds (default 30, messages older than N seconds before bridge connect are HELD instead of auto-dispatched — review via /wa-pending; set to 0 to restore auto-dispatch), egpt_chats [...JIDs treated as full-mirror egpt chats; self-DM is always one], mirror_headers ("all" default | "brain_only" — only persona/session items carry headers | "none"), follow_join ("never" default | "from_shell" adopt wa-join announces from a shell peer | "always" adopt from any peer), media { download ("off" default | "images_docs" save image+document attachments to ~/.egpt/media/<room>/<chatId>/<msgId>.<ext> | "all" save every media type), max_size_mb (default 25, skip downloads larger than this), summarize { operator (null default | session-name like "cgpt1" — auto-run the operator on each downloaded image/document and save a .summary.md alongside; PENDING — operator wiring not built yet, leave null for now), types (default ["image","document"]) }, audio_transcribe { enabled (default false — PENDING; Whisper/Claude-audio not wired; flagged to track interest) } } }',
