...
export const CONFIG_SCHEMA = {
  ...
  whatsapp_cdp: 'extension-only WhatsApp via a content script in web.whatsapp.com. v1: { enabled (set to false to opt out), chat_id (auto-captured), chat_name (the visible label of your "Write to yourself" / canonical egpt chat — e.g. "An". When set, dispatch fires for every message in THAT chat, but messages in any OTHER chat require a leading "@egpt" or "@e" wake-word to act on — safer default. When unset, every chat dispatches everything), client_name (default "wa-cdp") }. Tab presence IS the on/off switch. Background republishes incoming WA messages as room-utterance bus events. See BRIDGES_CDP_SPEC.md.',
