...
<room>/
  ...
  transcript.md    first-class (I3); rotates with the THREAD, not on a timer
  transcripts/     past threads, archived as <old-thread-id>.md on a reset
...
- **The idle TTL is config, per class.** `warm.idle_ttl_by_class.conversation`
  ...
  (`warm: { idle_ttl: 1h }`, or `-1` = never idle-evict; `0` means ALWAYS evict
  since the 2026-07-26 dialect flip, so it is the opposite of what it once was).
