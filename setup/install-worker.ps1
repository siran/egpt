...
if ($existingRule) {
  Write-Host "Updating existing firewall rule '$ruleName' -> TCP $Port (Private)..." -ForegroundColor Cyan
  ...
  Write-Host "Creating firewall rule '$ruleName' -> allow inbound TCP $Port (Private profile)..." -ForegroundColor Cyan
...
# rule won't apply there by design).
...
Write-Host "  on the SPINE, set: transcription_endpoint = http://<this-machine-LAN-ip>:$Port  (+ the same transcription_token)"
